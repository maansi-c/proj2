...
static ir_node *expression_to_value_simple_call(expression_t const *const expr);
...
static ir_node *call_expression_to_firm(const call_expression_t *const call)
{
	...
	if (function->kind == EXPR_REFERENCE) {
		...
		if (entity->kind == ENTITY_FUNCTION) {
			...
			} else if (builtin == BUILTIN_NONE && is_declaration(entity)
	...
	if (!firm_builtin) {
		callee = simple_call ?
			expression_to_value_simple_call(function)
			: expression_to_value(function);
	}
...
static ir_node *expression_to_value_simple_call(expression_t const *const expr)
{
	...
	assert(expr->kind == EXPR_REFERENCE);

	const reference_expression_t *ref    = &expr->reference;
	dbg_info *const dbgi   = get_dbg_info(&ref->base.pos);
	entity_t *const entity = ref->entity;

	assert(is_declaration(entity));
	assert(entity->kind == ENTITY_FUNCTION);
	assert(entity->function.btk == BUILTIN_NONE);
	assert((declaration_kind_t)entity->declaration.kind == DECLARATION_KIND_FUNCTION);
        
	ir_node *const addr = new_d_Address(dbgi, entity->function.irentity);
	return deref_address(dbgi, entity->declaration.type, addr);
}
