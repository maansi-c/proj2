...
typedef struct wlk_env {
	...
	ir_node             **lowered_params;  /**< Cache for params already lowered before. */
} wlk_env;
...
{
	if (env->lowered_params[pn] != NULL) {
		return env->lowered_params[pn];
	}
	...
	ir_node *result = new_rd_Member(dbgi, block, frame, compound);

	env->lowered_params[pn] = result;
...
static void transform_irg(lowering_env_t const *const env, ir_graph *const irg)
{
	...
	wlk_env walk_env = {
		...
		.lowered_params = NEW_ARR_D(ir_node*, env->obst, get_method_n_params(lowered_mtp)),
		.only_local_mem = true,
	};

	memset(walk_env.lowered_params, 0, ARR_LEN(walk_env.lowered_params) * sizeof(ir_node*));
