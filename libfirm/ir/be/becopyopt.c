...
static copy_opt_t *new_copy_opt(be_chordal_env_t *chordal_env, cost_fct_t get_costs)
{
	...
	co->get_compression_reqs = chordal_env->get_compression_reqs;
...
static void add_edge(copy_opt_t *co, ir_node *n1, ir_node *n2, int costs, aff_edge_type_t edge_type)
{
...
static inline void add_edges(copy_opt_t *co, ir_node *n1, ir_node *n2, int costs, aff_edge_type_t edge_type)
{
	if (n1 != n2 && !be_values_interfere(n1, n2)) {
		add_edge(co, n1, n2, costs, edge_type);
		add_edge(co, n2, n1, costs, edge_type);
	}
}
...
static void add_compression_req_edges(ir_node *irn, copy_opt_t *copyopt_env)
{
	...
	compression_req_t requirement = comp_req_fct(irn);

	switch (requirement.req_type) {
		case comp_req_2addr:
		case comp_req_2addr_register_subset: {
			// If the operand is not set, there is nothing to do
			if (requirement.n_op1 < 0) break;

			int weight = copyopt_env->get_costs(irn, 0);

			if (requirement.n_op1 >= 0) {
				ir_node *src = get_irn_n(irn, requirement.n_op1);
				add_edges(copyopt_env, irn, src, weight, aff_edge_compression);
			}

			// If the requirement is commutative, we also insert an affinity
			// with the second operand (if it is set)
			if (requirement.commutative && requirement.n_op2) {
				ir_node *src2 = get_irn_n(irn, requirement.n_op2);
				add_edges(copyopt_env, irn, src2, weight, aff_edge_compression);
...
static void build_graph_walker(ir_node *irn, void *env)
{
	...
	// Check whether we need to add edges to account for a requirement that specifies
	// a source/destination 2-address requirement
	add_compression_req_edges(irn, co);
	...
	if (is_Phi(irn)) { /* Phis */
		foreach_irn_in(irn, pos, arg) {
			add_edges(co, irn, arg, co->get_costs(irn, pos), aff_edge_noncopy);
		}
	} else if (is_Perm_Proj(irn)) { /* Perms */
		ir_node *arg = get_Perm_src(irn);
		add_edges(co, irn, arg, co->get_costs(irn, -1), aff_edge_noncopy);
	} else if (req->should_be_same != 0) {
		const unsigned other = req->should_be_same;
		for (int i = 0; 1U << i <= other; ++i) {
			if (other & (1U << i)) {
				...
				if (!arch_irn_is_ignore(other))
					add_edges(co, irn, other, co->get_costs(irn, -1), aff_edge_noncopy);
			}
		}
	}
}
