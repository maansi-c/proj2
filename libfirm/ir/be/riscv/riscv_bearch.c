...
static ir_settings_arch_dep_t riscv_arch_dep = {
	.replace_muls         = true,
	.replace_divs         = true,
	.replace_mods         = true,
	.allow_mulhs          = true,
...
unsigned riscv_machine_size = 32;
unsigned riscv_register_size = 4;
unsigned riscv_param_stack_align = 4;
...
static bool riscv_isa_has_fpu(riscv_isa_t isa)
{
	return isa == rv32imafd || isa == rv64imafd;
}

static bool riscv_abi_has_hardfloat(riscv_abi_t abi)
{
	return abi == ilp32d || abi == lp64d;
}
...
static void riscv_init(void)
{
	...
	use_softfloat = !riscv_isa_has_fpu((riscv_isa_t)isa);
	if (use_softfloat && riscv_abi_has_hardfloat((riscv_abi_t)abi)) {
		panic("requested ABI requires -march to subsume the 'D' extension");
	} else if (!use_softfloat && !riscv_abi_has_hardfloat((riscv_abi_t)abi)) {
...
static ir_node *riscv_new_reg_store(dbg_info *dbgi, ir_node *block, ir_node *mem, ir_node *base, ir_node *value,
                                    ir_entity *const ent, int32_t const val)
{
	if (riscv_is_64()) {
		return new_bd_riscv_sd(dbgi, block, mem, base, value, ent, val);
	} else {
		return new_bd_riscv_sw(dbgi, block, mem, base, value, ent, val);
	}
}

static ir_node *riscv_new_reg_load(dbg_info *dbgi, ir_node *block, ir_node *mem, ir_node *base,
                                   ir_entity *const ent, int32_t const val)
{
	if (riscv_is_64()) {
		return new_bd_riscv_ld(dbgi, block, mem, base, ent, val);
	} else {
		return new_bd_riscv_lw(dbgi, block, mem, base, ent, val);
	}
}

typedef ir_node *cons_store(dbg_info*, ir_node*, ir_node*, ir_node*, ir_node*, ir_entity *const, int32_t);
...
static ir_node *riscv_new_spill(ir_node *const value, ir_node *const after)
{
	...
	if (be_mode_needs_gp_reg(mode)) {
		...
		ir_node    *const store = riscv_new_reg_store(NULL, block, nomem, frame, value, NULL, 0);
...
typedef ir_node *cons_load(dbg_info*, ir_node*, ir_node*, ir_node*, ir_entity *const, int32_t);
...
static ir_node *riscv_new_reload(ir_node *const value, ir_node *const spill, ir_node *const before)
{
	...
	if (be_mode_needs_gp_reg(mode)) {
		...
		ir_node   *const frame = get_irg_frame(irg);
		ir_node   *const load  = riscv_new_reg_load(NULL, block, spill, frame, NULL, 0);
...
static regalloc_if_t const riscv_regalloc_if = {
	...
	.get_op_compression_requirements = riscv_get_op_compression_requirements,
};
...
static void riscv_collect_frame_entity_nodes(ir_node *const node, void *const data)
{
	...
	if ((riscv_is_32() && is_riscv_lw(node)) ||
	    (riscv_is_64() && is_riscv_ld(node))) {
...
static void riscv_introduce_prologue(ir_graph *const irg, unsigned size, bool omit_fp, bool is_variadic)
{
	...
	if (!omit_fp) {
		...
		if (!is_simm12(aligned)) {
			...
			/* save fp to stack */
			ir_node *const store_fp = riscv_new_reg_store(NULL, block, mem, add, start_fp, NULL, fp_save_offset);
			...
			/* save fp to stack */
			ir_node *const store_fp = riscv_new_reg_store(NULL, block, mem, start_sp, start_fp, NULL, aligned + fp_save_offset);
...
static void riscv_introduce_epilogue(ir_node *const ret, unsigned const size, bool omit_fp, bool is_variadic)
{
	...
	if (!omit_fp) {
		...
		/* restore old fp */
		int fp_save_offset = is_variadic ? -(RISCV_N_PARAM_REGS + 1) * riscv_register_size : -riscv_register_size;
		ir_node *const load_old_fp = riscv_new_reg_load(NULL, block, curr_mem, curr_fp, NULL, fp_save_offset);
...
static void riscv_lower_for_target(void)
{
	...
	foreach_irp_irg(i, irg) {
		// We always use a 32-bit mode here because
		// lower_switch does not like to handle 64 bit
		// selectors.
...
{
	if (xlen == 64) {
		riscv32_isa_if.pointer_size = 8;
		// TODO This is not correct, but the middle-end doesn't like 32 bit modes with 64
		// bit modulo-shifts: The front-end builds nodes with a new mode "U32" (with modulo
		// shift 64), the middle-end then mixes in mode_Iu, and things predictably break.
		// We have a special case in gen_shift_op (riscv_transform.c) that emulates modulo
		// shift 32 using the ...w instructions.
		riscv32_isa_if.modulo_shift = 32;

		riscv_machine_size = 64;
		riscv_register_size = 8;
		riscv_param_stack_align = 8;

		riscv_arch_dep.max_bits_for_mulh = 64;
