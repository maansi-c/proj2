...
static void emit_long_immediate(ir_node const *const node)
{
	riscv_immediate_attr_t const *const imm = get_riscv_immediate_attr_const(node);
	assert(imm->ent == NULL);
	be_emit_irprintf("%" PRId64, imm->val);
}
...
void riscv_emitf(ir_node const *const node, char const *fmt, ...)
{
	BE_EMITF(node, fmt, ap, false) {
		...
		case 'K': emit_long_immediate(node); break;
...
static void emit_be_MemPerm(ir_node const *const node)
{
	...
	char const *const frame_base = omit_fp ? "sp" : "s0";
...
static void emit_riscv_jalr(ir_node const *const node)
{
	...
	// get the arity of the function -> used to determine the arity
...
static void riscv_add_compressibility_info(ir_node *const node)
{
	...
	compression_req_t req = riscv_get_op_compression_requirements(node);

	if (req.req_type == comp_req_register_subset || req.req_type == comp_req_2addr_register_subset) {
		register_note = "comp_req=register";
	}

	if (req.req_type == comp_req_2addr || req.req_type == comp_req_2addr_register_subset) {
		addr_note = "comp_req=address";
	}
