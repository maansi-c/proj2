...
static riscv_addr make_addr(ir_node *addr)
{
	...
	if (is_Add(addr)) {
		...
		if (is_Const(r)) {
			ir_tarval const *const v = get_Const_tarval(r);
			if (tarval_is_simm12(v)) {
				val  = get_tarval_long(v);
...
{
	/* We assume the "medlow" code model here, where the binary
	 * only occupies the low 2 GB of address space. */
...
typedef ir_node *cons_binop_imm(dbg_info*, ir_node*, ir_node*, ir_entity*, int64_t);

static ir_node *new_binop_32_64(ir_mode *mode, cons_binop *cons_normal, cons_binop *cons_w,
                                dbg_info *dbgi, ir_node *block, ir_node *l, ir_node *r)
{
	if (riscv_is_64()) {
		if (get_mode_size_bits(mode) == 64) {
			return cons_normal(dbgi, block, l, r);
		} else {
			return cons_w(dbgi, block, l, r);
		}
	} else {
		return cons_normal(dbgi, block, l, r);
	}
...
static ir_node *new_binop_imm_32_64(ir_mode *mode, cons_binop_imm *cons_normal, cons_binop_imm *cons_w,
                                dbg_info *dbgi, ir_node *block, ir_node *l, ir_entity *const ent, int32_t const val)
{
	if (riscv_is_64()) {
		if (get_mode_size_bits(mode) == 64) {
			return cons_normal(dbgi, block, l, ent, val);
		} else {
			return cons_w(dbgi, block, l, ent, val);
		}
	} else {
		return cons_normal(dbgi, block, l, ent, val);
	}
...
static bool riscv_check_immediate_constraint(ir_tarval const *const val, char const imm_type)
{
	switch (imm_type) {
	case 'I': return tarval_is_simm12(val);
	case 'J': return val == 0;
	case 'K': return is_uimm5_6(get_tarval_long(val));
...
static bool riscv_match_immediate(riscv_asm_operand_t *const operand, ir_node *const node, char const imm_type)
{
	...
	long value;
	if (offset) {
		if (!riscv_check_immediate_constraint(offset, imm_type))
			return false;
...
static ir_node *gen_Add(ir_node *const node)
{
	...
	if (be_mode_needs_gp_reg(mode)) {
		...
		if (is_Const(r)) {
			ir_tarval const *const val = get_Const_tarval(r);
			if (tarval_is_simm12(val))
				return new_binop_imm_32_64(mode, new_bd_riscv_addi, new_bd_riscv_addiw,
				                           dbgi, block, new_l, NULL, get_tarval_long(val));
		}
		ir_node *const new_r = be_transform_node(r);
		return new_binop_32_64(mode, new_bd_riscv_add, new_bd_riscv_addw,
		                       dbgi, block, new_l, new_r);
	}
	TODO(node);
}
...
static ir_node *gen_logic_op(ir_node *const node, cons_binop *const cons, cons_binop_imm *const cons_imm)
{
       ...
       if (is_Const(r)) {
	       ir_tarval const *const val = get_Const_tarval(r);
               if (tarval_is_simm12(val))
	               return cons_imm(dbgi, block, new_l, NULL, get_tarval_long(val));
       }
       ir_node *const new_r = be_transform_node(r);
...
static ir_node *gen_saturating_increment(ir_node *const node)
{
  ...
  ir_mode  *const mode    = get_irn_mode(operand);
  assert(get_mode_size_bits(mode) == 32 || get_mode_size_bits(mode) == 64);

  ir_node *const sltiu = new_bd_riscv_sltiu(dbgi, block, operand, NULL, -1);
  ir_node *const addu  = new_binop_32_64(mode, new_bd_riscv_add, new_bd_riscv_addw, dbgi, block, operand, sltiu);
  return addu;
}
...
static ir_node *gen_Const(ir_node *const node)
{
	ir_mode *const mode = get_irn_mode(node);
	if (be_mode_needs_gp_reg(mode)) {
		int64_t const val = (int64_t)get_tarval_uint64(get_Const_tarval(node));
		if (val == 0) {
			ir_graph *const irg = get_irn_irg(node);
			...
			if (get_mode_size_bits(mode) == 64 &&
				...
				// TODO Assembling large constants
				// using lui/addi is really not
				// sensible. We should learn to build
				// constant pools.

				// TODO Also, we use a
				// pseudo-instruction here for the
				// time being because I can't be
				// bothered to work out the bit
				// fiddling.
				return new_bd_riscv_li(dbgi, block, NULL, val);
			}
...
static ir_node *gen_Div(ir_node *const node)
{
	...
	unsigned modebits = get_mode_size_bits(mode);
	if (be_mode_needs_gp_reg(mode) &&
	    (modebits == riscv_machine_size || (riscv_is_64() && modebits == 32))) {
		dbg_info *const dbgi  = get_irn_dbg_info(node);
		ir_node  *const block = be_transform_nodes_block(node);
		ir_node  *const l     = be_transform_node(get_Div_left(node));
		ir_node  *const r     = be_transform_node(get_Div_right(node));
		if (mode_is_signed(mode)) {
			return new_binop_32_64(mode, new_bd_riscv_div, new_bd_riscv_divw, dbgi, block, l, r);
		} else {
			return new_binop_32_64(mode, new_bd_riscv_divu, new_bd_riscv_divuw, dbgi, block, l, r);
		}
	}
	TODO(node);
}
	...
	if (be_mode_needs_gp_reg(mode)) {
		...
		if (size == 8) {
			...
			if (riscv_is_32()) {
				...
				cons = mode_is_signed(mode) ? &new_bd_riscv_lw : &new_bd_riscv_lwu;
			}
		} else if (size == 64 && riscv_is_64()) {
			cons = new_bd_riscv_ld;
		} else {
			panic("invalid load");
		}
		dbg_info  *const dbgi  = get_irn_dbg_info(node);
...
static ir_node *gen_Minus(ir_node *const node)
{
	...
	if (be_mode_needs_gp_reg(mode)) {
		...
		return new_binop_32_64(mode, new_bd_riscv_sub, new_bd_riscv_subw, dbgi, block, new_l, new_r);
	}
	TODO(node);
}
...
static ir_node *gen_Mod(ir_node *const node)
{
	...
	if (be_mode_needs_gp_reg(mode) &&
		...
		if (mode_is_signed(mode)) {
			return new_binop_32_64(mode, new_bd_riscv_rem, new_bd_riscv_remw, dbgi, block, l, r);
		} else {
			return new_binop_32_64(mode, new_bd_riscv_remu, new_bd_riscv_remuw, dbgi, block, l, r);
		}
	}
	TODO(node);
}
...
static ir_node *gen_Mul(ir_node *const node)
{
	...
	if (be_mode_needs_gp_reg(mode)) {
		...
		return new_binop_32_64(mode, new_bd_riscv_mul, new_bd_riscv_mulw, dbgi, block, l, r);
	}
	TODO(node);
}
...
static ir_node *gen_Mulh(ir_node *const node)
{
	...
	if (be_mode_needs_gp_reg(mode)) {
		dbg_info *const dbgi  = get_irn_dbg_info(node);
		ir_node  *const block = be_transform_nodes_block(node);
		if (get_mode_size_bits(mode) == riscv_machine_size) {
		...
		} else if (riscv_is_64() && get_mode_size_bits(mode) == 32) {
			ir_node *const l = make_extension(dbgi, get_Mulh_left(node), 64);
			ir_node *const r = make_extension(dbgi, get_Mulh_right(node), 64);
			ir_node *const mul64 = new_bd_riscv_mul(dbgi, block, l, r);
			// RV64 always demands sign-extension, so this
			// instruction is independent of the mode.
			return new_bd_riscv_srai(dbgi, block, mul64, NULL, 32);
		}
	}
	TODO(node);
}
...
static ir_node *gen_Proj_Proj_Start(ir_node *const node)
{
	...
	if (param->reg) {
		...
		ir_node  *const load  = riscv_is_64() ?
			new_bd_riscv_ld(dbgi, block, mem, base, param->entity, 0) :
			new_bd_riscv_lw(dbgi, block, mem, base, param->entity, 0);
...
static ir_node *gen_shift_op(ir_node *const node, cons_binop *const cons_normal, cons_binop_imm *const cons_imm_normal,
                             cons_binop *const cons_w, cons_binop_imm *const cons_imm_w, bool needs_extension)
{
	dbg_info *const dbgi  = get_irn_dbg_info(node);
	ir_node  *const block = be_transform_nodes_block(node);
	ir_node  *const l     = get_binop_left(node);
	...
	// TODO For now, we must correctly emulate modulo shift 32 for modes < 64 bit wide.
	// See comment in riscv_set_xlen (riscv_bearch.c).
	bool emulate_32 = riscv_is_64() && size < 64;

	cons_binop     *const cons     = emulate_32 ? cons_w     : cons_normal;
	cons_binop_imm *const cons_imm = emulate_32 ? cons_imm_w : cons_imm_normal;
	unsigned        const regsize  = emulate_32 ? 32         : riscv_machine_size;

	needs_extension = needs_extension && (size != regsize);
	int extend = regsize - size;
	/* right shift operations with mode sizes < XLEN Bit require correct sign/zero extension.
	...
	if (is_Const(r)) {
		...
		if (needs_extension && is_uimm5_6(extend + val)) {
			val = extend + val;
			needs_extension = false;
		}
		if (is_uimm5_6(val)) {
...
static ir_node *gen_Switch(ir_node *const node)
{
	...
	ir_node   *const sll    = new_bd_riscv_slli(dbgi, block, sel, NULL, riscv_is_64() ? 3 : 2);
