...
static aggregate_spec_t classify_for_ilp32(ir_type const *const type) {
	...
	/* if type has 8 byte alignment and result is two (4-byte) mode_rv_gp slices we have to convert this into
	...
	if (result.length == 2 && result.modes[0] == mode_rv_gp && result.modes[1] == mode_rv_gp &&
